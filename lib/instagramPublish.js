...
async function publishToInstagramDirect({ videoUrl, caption, isTrial = false }) {
...
// isTrial игнорируется на Postproxy-пути (там нет trial_params) — при сетевом
// сбое лучше обычный релиз, чем незапощенный пост.
async function publishToInstagram({ videoUrl, caption, isTrial = false }) {
  try {
    return await publishToInstagramDirect({ videoUrl, caption, isTrial });
  } catch (err) {
