...
async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = getPool().query(`
      ...
      ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS media_type TEXT NOT NULL DEFAULT 'video';
      ALTER TABLE scheduled_posts ADD COLUMN IF NOT EXISTS is_trial BOOLEAN NOT NULL DEFAULT false;
    `);
  }
  return schemaReady;
}
...
// mediaType='carousel' (Instagram image carousel, see apd-stroy-instagram-norms-carousel-series):
// videoUrl holds JSON.stringify(imageUrls) instead of a single video URL — reuses the existing
// NOT NULL video_url column rather than adding a nullable one, instagramPublish.js branches on media_type.
async function enqueuePost({ reelSlug, videoUrl, caption, scheduledAt, platform = 'instagram', contentTrack = 'main', mediaType = 'video', isTrial = false }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `INSERT INTO scheduled_posts (reel_slug, video_url, caption, scheduled_at, platform, content_track, media_type, is_trial) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
    [reelSlug, videoUrl, caption, scheduledAt, platform, contentTrack, mediaType, isTrial]
  );
  return rows[0].id;
}
...
// Серия карусель "Стройка по нормам" (content_track='norms_carousel', см. память
// apd-stroy-instagram-norms-carousel-series): ежедневно, отдельный трек от
// main/active_cta — явное требование пользователя 01.09.2026 ("каждый день")
// после того как session-only CronCreate-расписание сорвалось (карусели 02 и 03
// не опубликовались, т.к. сессия/ноутбук были недоступны в момент срабатывания).
// postHourUTC=13 -> 18:00 Пермь (UTC+5), близко к прежнему ручному расписанию
// (18:30/19:02) и не совпадает по часу с main (7 UTC), active_cta (8 UTC),
// YouTube (10 UTC) или VK (11 UTC).
async function getNextNormsCarouselSlot(postHourUTC = 13) {
  return getNextWeekdaySlot('instagram', [0, 1, 2, 3, 4, 5, 6], postHourUTC, 'norms_carousel');
}

// Атомарный "claim": SELECT и переход в status='publishing' происходят одним
// запросом (SELECT ... FOR UPDATE SKIP LOCKED внутри подзапроса + UPDATE).
// До этого исправления (19.09.2026) SELECT и последующий markPublishing()
// были двумя отдельными запросами — при двух параллельных вызовах
// processDuePosts (напр. локальный одноразовый скрипт публикации + прод-кроном
// /cron/scheduled-publish, тикающим на хостинге раз в 30 мин, оба работают с
// ОДНОЙ базой) оба могли выбрать одни и те же ещё-pending строки до того как
// любой из них проставит 'publishing' — реальный баг, вызвавший дублирование
// публикации карусели «Стройка по нормам» 17.09.2026 (утеплитель и мембрана
// ушли в Instagram по два раза с разницей ~1.5 мин). SKIP LOCKED гарантирует,
// что конкурентный вызов возьмёт другие строки, а не те же самые.
async function getDuePosts(limit = 5, platform = null) {
  await ensureSchema();
  const { rows } = platform
    ? await getPool().query(
        `UPDATE scheduled_posts SET status = 'publishing' WHERE id IN (
           SELECT id FROM scheduled_posts
           WHERE status = 'pending' AND scheduled_at <= now() AND platform = $2
