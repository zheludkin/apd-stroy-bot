...
// hard API rejection above that — the ' #Shorts' suffix (8 chars) and the
// ellipsis both count against that cap); the whole caption becomes the description.
function splitCaption(caption) {
  const firstLine = caption.split('\n')[0].trim();
  const suffix = ' #Shorts';
  const maxContentLen = 100 - suffix.length;
  const title = (firstLine.length > maxContentLen ? firstLine.slice(0, maxContentLen - 1) + '…' : firstLine) + suffix;
