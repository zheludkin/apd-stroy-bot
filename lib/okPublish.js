...
async function okCall(method, extraParams, httpMethod = 'GET', body = null) {
  ...
  const all = { ...params, sig, access_token: process.env.OK_SESSION_KEY };
  let res;
  if (httpMethod === 'POST' && !body) {
    // Параметры POST — в теле, а не в адресе: attachment карусели (7 фото-токенов +
    // длинная подпись) не помещался в URL, и ОК рвал соединение («fetch failed» /
    // HTML-ошибка вместо JSON) — так 25.09.2026 не ушла карусель статьи 2.
    ...
    res = await fetch(url, { method: httpMethod, body });
  }
