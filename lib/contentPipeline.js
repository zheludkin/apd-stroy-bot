...
async function upsertStage(reelSlug, stage, fields = {}) {
  ...
  if (!existing) {
    await getPool().query(
      `INSERT INTO content_pipeline (reel_slug, stage, video_url, telegram_chat_id, telegram_message_id, error, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [reelSlug, stage, fields.videoUrl || null, fields.telegramChatId || null, fields.telegramMessageId || null, fields.error || null, fields.notes || null]
    );
    return;
  }
  await getPool().query(
...
async function listPipeline() {
  await ensureSchema();
  const { rows } = await getPool().query(`SELECT reel_slug, stage, updated_at, error FROM content_pipeline ORDER BY id`);
  return rows;
}
