...
function readProjectKnowledge() {
  try {
    return fs.readFileSync(
      path.join(__dirname, '..', 'claude-project-knowledge.md'),
      'utf8'
    );
  } catch (e) {
    return '(файл claude-project-knowledge.md не найден в репозитории)';
  }
}
...
=== Накопленные знания по проекту (для быстрого подключения на новой машине) ===
Куратский снимок того, чему Claude Code научился за время работы над этим
проектом — правила контента, техническая архитектура, известные блокеры.
Обновляется вручную время от времени, не автоматически.
